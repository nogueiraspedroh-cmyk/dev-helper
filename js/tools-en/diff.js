// js/tools-en/diff.js — logic for Text Diff (English version).
// Loaded ONLY on en/tools/diff/index.html, after js/main.js.
// Mirrors js/tools/diff.js — same logic, only user-visible strings translated.
...
(function () {
  ...
  function diffText(textA, textB, mode) {
    ...
    if ((a.length + 1) * (b.length + 1) > MAX_CELLS) {
      return {
        error: "The texts are too large to compare in this mode. " +
          "Try line mode or reduce the content.",
        ops: [],
  ...
  function compare() {
    ...
    var unit = mode === "word" ? "word(s)" : "line(s)";
    statsEl.textContent =
      result.stats.added + " added, " +
      result.stats.removed + " removed " + unit +
      (result.stats.added === 0 && result.stats.removed === 0 ? " — the texts are identical." : ".");
  }
