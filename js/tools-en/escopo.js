// js/tools-en/escopo.js — Software Project Scope Generator (English version).
// Loaded ONLY on en/tools/escopo/index.html, after js/main.js.
// Mirrors js/tools/escopo.js — same logic, only user-visible strings translated.
...
(function () {
  ...
  var LISTAS = [
    { id: "lista-funcionais",    secao: "## Functional requirements" },
    { id: "lista-nao-funcionais",secao: "## Non-functional requirements" },
    { id: "lista-entregaveis",   secao: "## Deliverables" },
    { id: "lista-fora-escopo",   secao: "## Out of scope" },
    { id: "lista-premissas",     secao: "## Assumptions" },
    { id: "lista-riscos",        secao: "## Risks" },
    { id: "lista-criterios",     secao: "## Acceptance criteria" }
  ];
  ...
  function criarItemLista(listaEl) {
    ...
    input.placeholder = "Type the item...";
    ...
    btnRemover.setAttribute("aria-label", "Remove item");
  ...
  function montarMarkdown() {
    ...
    var tituloEfetivo = nome || "Untitled project";
    ...
    if (prazo || stack) {
      if (prazo) { linhas.push("**Estimated timeline:** " + prazo + "  "); }
      if (stack) { linhas.push("**Stack / Technologies:** " + stack + "  "); }
      linhas.push("");
    }

    // Objetivo
    if (objetivo) {
      linhas.push("## Objective");
      linhas.push("");
  ...
  function handleBaixar() {
    ...
    window.DevHelper.flashButton(btnBaixar, "Downloading...");
  }
