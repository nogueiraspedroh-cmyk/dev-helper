// js/tools-en/yaml-json.js — YAML <-> JSON converter (English version).
// Loaded ONLY on en/tools/yaml-json/index.html, after js/main.js.
// Mirrors js/tools/yaml-json.js — same logic, only user-visible strings translated.
...
(function () {
  ...
  function countIndent(line, lineNum) {
    ...
    if (line.charAt(i) === "\t") {
      throw new Error("Line " + lineNum + ": tabs are not allowed in YAML indentation. Use spaces.");
    }
    return i;
  }

  function tokenize(text) {
    if (typeof text !== "string") { throw new Error("Invalid YAML input."); }
  ...
  function parseScalar(s) {
    ...
    if ((s.charAt(0) === "[" && s.charAt(s.length - 1) === "]") ||
      ...
      throw new Error("Non-empty inline collections ([a, b] / {a: b}) are not supported by this simplified converter. Use the block form (indented).");
    }
    return s;
  }
  ...
  function splitKeyValue(content, lineNum) {
    ...
    throw new Error("Line " + lineNum + ": expected 'key: value'. Got: " + content);
  }
  ...
  function parseMap(lines, i, indent) {
    ...
    while (i < lines.length) {
      ...
      if (ln.indent > indent) {
        throw new Error("Line " + ln.line + ": unexpected indentation.");
      }
  ...
  function parseList(lines, i, indent) {
    ...
    while (i < lines.length) {
      ...
      if (ln.indent < indent) { break; }
      if (ln.indent > indent) {
        throw new Error("Line " + ln.line + ": unexpected indentation in list.");
      }
  ...
  function convert() {
    ...
    try {
      ...
      if (modeInfo) {
        modeInfo.textContent = result.mode === "json2yaml" ? "Detected: JSON → YAML" : "Detected: YAML → JSON";
      }
    } catch (e) {
