// js/tools-en/xml.js — XML Formatter/Adjuster (English version).
// Loaded ONLY on en/tools/xml/index.html, after js/main.js.
// Mirrors js/tools/xml.js — same logic, only user-visible strings translated.
...
(function () {
  ...
  function decodeEntities(raw, text, basePos) {
    ...
    while (i < raw.length) {
      ...
      if (ch === "&") {
        ...
        if (semi === -1) {
          throw new Error(
            "Unterminated entity (missing ';') near " + posInfo(text, basePos + i) + "."
          );
        }
        ...
        if (replaced === null) {
          throw new Error(
            "Unknown entity '&" + ent + ";' at " + posInfo(text, basePos + i) + "."
          );
        }
        out += replaced;
        i = semi + 1;
  ...
  function consumeDoctype(text, state) {
    ...
    throw new Error("Unclosed <!DOCTYPE ...> declaration (" + posInfo(text, start) + ").");
  }
  ...
  function parseComment(text, state) {
    ...
    if (end === -1) {
      throw new Error("Unclosed comment (started at " + posInfo(text, start) + ").");
    }
  ...
  function parseCData(text, state) {
    ...
    var end = text.indexOf("]]>", state.pos);
    if (end === -1) {
      throw new Error("Unclosed CDATA section (started at " + posInfo(text, start) + ").");
    }
    var value = text.slice(state.pos, end);
    state.pos = end + 3;
  ...
  function parseAttributes(text, state, tagName) {
    ...
    while (true) {
      ...
      if (attrName === "") {
        throw new Error(
          "Unexpected character in tag <" + tagName + "> at " + posInfo(text, state.pos) + "."
        );
      }
      skipWs(text, state);
      if (text.charAt(state.pos) !== "=") {
        throw new Error(
          "Attribute '" + attrName + "' has no value (expected '=') at " + posInfo(text, state.pos) + "."
        );
      }
      state.pos++; // "="
      skipWs(text, state);
      ...
      if (quote !== '"' && quote !== "'") {
        throw new Error(
          "Attribute '" + attrName + "' is not quoted (use single or double quotes) at " +
      ...
      if (closeIdx === -1) {
        throw new Error("Unclosed quotes in attribute '" + attrName + "' of tag <" + tagName + ">.");
      }
      var rawVal = text.slice(valStart, closeIdx);
      state.pos = closeIdx + 1;
      var value = decodeEntities(rawVal, text, valStart);
      if (seen[attrName]) {
        throw new Error("Duplicate attribute '" + attrName + "' in tag <" + tagName + ">.");
      }
      seen[attrName] = true;
  ...
  function parseElement(text, state) {
    ...
    if (name === "") {
      throw new Error("Empty tag name at " + posInfo(text, tagStart) + ".");
    }
    ...
    if (text.charAt(state.pos) !== ">") {
      throw new Error("Malformed tag <" + name + "> at " + posInfo(text, state.pos) + " (expected '>').");
    }
    state.pos++; // ">"
    ...
    while (true) {
      if (state.pos >= text.length) {
        throw new Error("Unclosed tag: <" + name + "> (opened at " + posInfo(text, tagStart) + ").");
      }
      if (startsWith(text, state, "</")) {
        ...
        skipWs(text, state);
        if (text.charAt(state.pos) !== ">") {
          throw new Error("Malformed closing tag </" + closeName + "> at " + posInfo(text, closeStart) + ".");
        }
        state.pos++;
        if (closeName !== name) {
          throw new Error(
            "Tag not closed correctly: expected </" + name + "> but found </" + closeName +
              "> at " + posInfo(text, closeStart) + "."
          );
        }
        break;
        ...
        if (piEnd === -1) {
          throw new Error("Unclosed processing instruction at " + posInfo(text, state.pos) + ".");
        }
        state.pos = piEnd + 2; // instruções de processamento são ignoradas (fora de escopo)
      } else if (startsWith(text, state, "<!")) {
        throw new Error("Unsupported declaration inside <" + name + "> at " + posInfo(text, state.pos) + ".");
  ...
  function parseXML(text) {
    ...
    if (startsWith(text, state, "<?xml")) {
      ...
      if (declEnd === -1) {
        throw new Error("Unclosed XML declaration (<?xml ...?>).");
      }
    ...
    while (true) {
      ...
      if (startsWith(text, state, "<!DOCTYPE")) {
        if (doctype !== null) {
          throw new Error("More than one <!DOCTYPE ...> declaration found.");
        }
    ...
    if (text.charAt(state.pos) !== "<") {
      if (state.pos >= text.length) {
        throw new Error("Empty XML document: no root element found.");
      }
      throw new Error("Text outside any element at " + posInfo(text, state.pos) + " (expected '<').");
    }
    ...
    while (state.pos < text.length) {
      ...
      if (ch === "<") {
        throw new Error("More than one root element found at " + posInfo(text, state.pos) + ".");
      }
      throw new Error("Text outside the root element at " + posInfo(text, state.pos) + ".");
    }
  ...
  function showError(err) {
    ...
    errorEl.textContent = "Syntax error: " + msg; // textContent — sem risco de XSS
