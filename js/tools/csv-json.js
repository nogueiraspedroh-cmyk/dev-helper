// js/tools/csv-json.js — lógica do conversor CSV <-> JSON.
// Carregado APENAS em tools/csv-json/index.html, após js/main.js.
...
(function () {
  ...
  function parseCSV(text, delimiter) {
    ...
    if (typeof text !== "string") {
      throw new Error("Entrada CSV inválida.");
    }
    var rows = [];
    var row = [];
    var field = "";
    ...
    if (inQuotes) {
      throw new Error("CSV malformado: aspas não fechadas (campo citado sem \" de fechamento).");
    }
  ...
  function structureToCsv(data, opts) {
    ...
    if (!Array.isArray(data)) {
      throw new Error("O JSON precisa ser um array (lista de objetos ou de arrays) para virar CSV.");
    }
    ...
    if (Array.isArray(data[0])) {
      for (i = 0; i < data.length; i++) {
        if (!Array.isArray(data[i])) {
          throw new Error("Array misto: se o primeiro item é um array, todos devem ser arrays.");
        }
    ...
    for (i = 0; i < data.length; i++) {
      ...
      if (obj === null || typeof obj !== "object" || Array.isArray(obj)) {
        throw new Error("Array misto: para gerar cabeçalho, todos os itens devem ser objetos.");
      }
      for (var k in obj) {
