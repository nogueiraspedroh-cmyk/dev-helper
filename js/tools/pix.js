// js/tools/pix.js — Gerador de PIX Copia e Cola + QR Code (BR Code / EMV).
// Carregado APENAS em tools/pix/index.html, DEPOIS de js/lib/qrcode-core.js e
// de js/main.js. Padrão defensivo: todo acesso ao DOM checa if (el).
// Anti-XSS: saída via .value / .textContent — nunca innerHTML com dado do usuário.
//
// Monta o payload BR Code (EMV) do PIX estático e calcula o CRC16-CCITT
// (poly 0x1021, init 0xFFFF, sem reflexão). O QR é gerado com o núcleo puro
// compartilhado em js/lib/qrcode-core.js (window.QRCore).
//
...
(function () {
  ...
  // ================================================================
  // NÚCLEO PURO — CRC16 + montagem do payload EMV
  // ================================================================

  // CRC-16/CCITT-FALSE: poly 0x1021, init 0xFFFF, sem reflexão, xorout 0x0000.
  // Vetor canônico documentado: crc16("123456789") === 0x29B1.
  function crc16(str) {
    var crc = 0xffff;
  ...
  // Normaliza texto para os campos de nome/cidade: maiúsculo, sem acento,
  // apenas caracteres seguros, truncado ao tamanho máximo.
  ...
  // Valor: aceita "10", "10,50" ou "10.50" -> "10.50". Vazio/0 -> "".
  ...
  // txid do campo 62/05: apenas alfanumérico, máx 25; "***" quando ausente.
  ...
  /**
   * Monta o payload BR Code (Copia e Cola) do PIX estático.
   * @param {{key:string,name:string,city:string,amount?:string,
   *          txid?:string,description?:string}} opts
   * @returns {string} payload EMV completo já com CRC16.
   * @throws {Error} se chave, nome ou cidade estiverem vazios.
   */
  function buildPixPayload(opts) {
    ...
    if (name === "") { throw new Error("Informe o nome do recebedor."); }
    if (city === "") { throw new Error("Informe a cidade do recebedor."); }

    var f00 = emv("00", "01");                 // Payload Format Indicator
    var f01 = emv("01", "11");                 // Point of Initiation (reutilizável)
    ...
    var f53 = emv("53", "986");                // Moeda (BRL)
    var f54 = amount !== "" ? emv("54", amount) : "";
    var f58 = emv("58", "BR");                 // País
    var f59 = emv("59", name);                 // Nome do recebedor
    var f60 = emv("60", city);                 // Cidade
