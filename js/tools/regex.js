// js/tools/regex.js — lógica do Regex Tester.
// Carregado APENAS em tools/regex/index.html, após js/main.js.
...
(function () {
  ...
  function renderGroups(matches) {
    ...
    matches.forEach(function (mt, mi) {
      ...
      addRow("#" + (mi + 1), "match completo", mt.match);
      anyRow = true;

      mt.groups.forEach(function (g, gi) {
        addRow("#" + (mi + 1), "grupo " + (gi + 1), typeof g === "undefined" ? "(sem captura)" : g);
      });

      if (mt.named) {
        Object.keys(mt.named).forEach(function (name) {
          var v = mt.named[name];
          addRow("#" + (mi + 1), "grupo <" + name + ">", typeof v === "undefined" ? "(sem captura)" : v);
        });
      }
    });
  ...
  function update() {
    ...
    if (result.error) {
      errorEl.textContent = "Regex inválida: " + result.error;
    ...
    if (patternEl.value === "") {
      countEl.textContent = "Digite um padrão para testar.";
      ...
      countEl.textContent =
        result.matchCount === 0
          ...
          : result.matchCount + (result.matchCount === 1 ? " correspondência." : " correspondências.");
    }
